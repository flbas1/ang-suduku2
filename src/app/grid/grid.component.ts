...
export class GridComponent implements OnInit {
	...
	useCell(cell) {
		...
		if (c.startsWith('N')) {
			...
			this.style.innerText = this.colorGrid();
		}
		else
	...
	onSolve() {
		if (this.__solve)
			this.__grid = this.grid.user;
		else
			this.__grid = this.grid.solved;
		this.__solve = !this.__solve;

		//var style = document.createElement('style');
		//this.style.innerText = '.C1 {background:yellow;}';
		//style.id='userWins'
		//document.body.appendChild(this.style);
		//document.body.removeChild(style);

		//var style = document.createElement('style');
		//document.getElementById("userWins").remove();
		//this.style.innerText += '.R1 {background:red;}';
		//style.id='userWins'
